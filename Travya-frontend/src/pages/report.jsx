...
export default function Report() {
  ...
  // Simple client-side guard: allow only police
  useEffect(() => {
    const role = localStorage.getItem('role') || ''
    if (role !== 'police') {
      setMessage('Access denied: Police only')
      // Stay on page to avoid redirect loop; backend also enforces role
    }
  }, [navigate])
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const resp = await fetch('http://localhost:5000/api/reports', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-User-Role': role || 'guest' },
